...
export class FileHubClient {
  ...
  private async api<T>(
    ...
    // Binary chunk bodies are passed through as-is. The ambient BodyInit type in
    // this project's Node config predates typed-array bodies, which undici has
    // supported for years, so the cast is a typings gap rather than a real one.
    const requestBody =
      body === undefined ? undefined : isBuffer ? (body as unknown as BodyInit) : JSON.stringify(body)
